...
TEST_SUITE("Aplicacao Principal - Protegido contra Loop Infinito") {
    ...
    TEST_CASE("1. Interativo - Erros e Criacao de Usuario") {
        std::remove("data/usuarios.csv"); std::remove("data/perfis.csv"); std::remove("data/comunidades.csv"); std::remove("data/posts.csv"); std::remove("data/comentarios.csv");
        std::string inputs = 
            "abc\n"
            "1\nerrado\nerrado\n2\n" 
            "2\nUser\nemail@uni.br\nSenha123\n" 
    ...
    TEST_CASE("4. Modo Automatico (Demonstracao)") {
        { RedirecionarIOApp io(""); Aplicacao app(true); CHECK_NOTHROW(app.executar()); } 
        ...
        { RedirecionarIOApp io("2\nNovoNome\nja@existe.com\nSenha123\n"); Aplicacao app(true); CHECK_THROWS_WITH(app.executar(), "EXIT_0"); }
        { RedirecionarIOApp io("2\nJa Existe\nnovo@mail.com\nSenha123\n"); Aplicacao app(true); CHECK_THROWS_WITH(app.executar(), "EXIT_1"); }
    }
