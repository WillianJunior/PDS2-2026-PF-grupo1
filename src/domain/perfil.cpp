...
Perfil::Perfil(std::string curso, std::string instituicao, int periodo)
    : id(0), emailUsuario(""), nome(""), descricao(""), curso(std::move(curso)), instituicao(std::move(instituicao)), periodo(periodo) {}
...
std::vector<int> Perfil::listarIdSeguidores() const { return idsSeguidores; }
std::vector<int> Perfil::listarIdSeguidos() const { return idsSeguidos; }
